...
import me.eldodebug.soar.discord.ipc.entities.*;
...
public final class IPCClient implements Closeable {
    ...
    public void connect(DiscordBuild... preferredOrder) throws NoDiscordClientException {
        ...
        log.debug("Client is now connected and ready!");
    ...
    public void sendRichPresence(RichPresence presence, Callback callback) {
        ...
        log.debug("Sending RichPresence to discord: {}", presence == null ? null : presence.toJson().toString());
    ...
    public void subscribe(Event sub, Callback callback) {
        ...
        log.debug("Subscribing to Event: {}", sub.name());
        JsonObject jsonObject = new JsonObject();
    ...
    public void close() {
        ...
        try {
            ...
            log.debug("Failed to close pipe", e);
        }
    }
    ...
    private void startReading() {
        ...
        readThread = new Thread(() -> {
            try {
                ...
                while ((p = pipe.read()).getOp() != OpCode.CLOSE) {
                    ...
                    switch (event) {
                        ...
                        case ERROR:
                            ...
                            break;
                        case ACTIVITY_JOIN:
                            log.debug("Reading thread received a 'join' event.");
                            break;

                        case ACTIVITY_SPECTATE:
                            log.debug("Reading thread received a 'spectate' event.");
                            break;

                        case ACTIVITY_JOIN_REQUEST:
                            log.debug("Reading thread received a 'join request' event.");
                            break;

                        case UNKNOWN:
                            log.debug("Reading thread encountered an event with an unknown type: " +
                ...
                log.error("Reading thread encountered an IOException", ex);
        ...
        log.debug("Starting IPCClient reading thread!");
        readThread.start();
    }
