...
public class MicrosoftAuthentication {
    ...
    private void getMicrosoftToken(@NotNull URL tokenURL) {
    ...
    private void getXSTS(String token, String refreshToken) {
        ...
        if (response.has("XErr")) {
            switch (response.get("XErr").getAsString()) {
                case "2148916233":
                    log.error("This account doesn't have an Xbox account.");
                    break;
                case "2148916235":
                    log.error("Xbox isn't available in your country.");
                    break;
                case "2148916238":
                    log.error("The account is under 18 and must be added to a Family (https://start.ui.xboxlive.com/AddChildToFamily)");
                    break;
            }
        } else {
    ...
    private void checkMinecraftOwnership(String token, String refreshToken) {
        ...
        if (!ownsMinecraft) {
            log.error("User doesn't own Minecraft");
        } else {
